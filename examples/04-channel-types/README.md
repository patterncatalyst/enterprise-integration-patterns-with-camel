...
1. **Demo data generator** producing orders every 5 seconds with rotating statuses (`placed`, `cancelled`, `refunded`)
2. **Point-to-Point** — each order consumed once and forwarded to `eip.orders.processed`
3. **Pub/Sub fan-out** — the same order event logged by all three subscribers (inventory, notification, analytics)
